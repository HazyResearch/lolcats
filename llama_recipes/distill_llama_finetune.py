...
def main():
    ...
    if 'lora' in args.model_config:  # a bit hacky, but we should name model_config to indicate peft
        if args.verbose and (rank == 0 or not args.enable_fsdp):
            print_header('*** Checkpoint Model ***')
            print(model)
            print_header('** Sanity check model weights **')
            for n, p in model.named_parameters():
                if ('layers.0.' in n and ('feature_map' in n or 'lora' in n)):
        ...
        if args.verbose and (rank == 0 or not args.enable_fsdp):
            print_header('*** Checkpoint Model Post-Merge ***')
            print(model)
