...
class LossComputer():
    ...
    def compute_loss(self, model: torch.nn.Module, inputs: torch.Tensor) -> torch.Tensor:
        ...
        loss_mse = 0
        loss_xent = 0
        n_layers = 1  # Number of layers to distill
        # y_true, attn_weights, past_key_value
        y_true, attn_weights, past_key_value = model(inputs[0], output_attentions=True, use_cache=False)      
        # Cross-entropy loss
        a_pred, a_true = attn_weights[0]
