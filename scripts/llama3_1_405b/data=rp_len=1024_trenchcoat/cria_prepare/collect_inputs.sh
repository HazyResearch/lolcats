...
#SBATCH --nodes=2
#SBATCH --nodelist=mk-xii-06,mk-xii-22                # TODO change to your nodenames
#SBATCH --gres=gpu:8
...
export MASTER_HOSTNAME="mk-xii-06"                      # # TODO change to your nodenames
...
srun  torchrun --nnodes 2 --node_rank $SLURM_NODEID --rdzv_id $RANDOM --rdzv_backend c10d --rdzv_endpoint $MASTER_ADDR:$MASTER_PORT --nproc_per_node 8 \
    --llama_recipes/trenchcoat_lolcat/save_llama_attn_inputs.py \
