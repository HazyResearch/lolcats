...
"""
Modified for linearizing layers
"""
import os
import functools
...
def fsdp_auto_wrap_policy(model, transformer_layer_name):
    """
    Return custom wrapping for FSDP to train linearizing layers
    """

    def lambda_policy_fn(module):
        if (
            len(list(module.named_children())) == 0 and (
                getattr(module, "weight", None) is not None or
                getattr(module, "layer", None) is not None
                # or str(type(module)) == "<class 'torch.nn.parameter.Parameter'>"
            )  # and module.weight.requires_grad  # (MZ 3/12/24) We want all modules for attention training
        ):
            return True
            # try:
