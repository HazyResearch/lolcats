...
llama_recipes/save_llama_outputs.py \
...
--finetune_config finetune_lora_qkvo_alpca_clean_llama3_1_70b \
--verbose --replicate 4 --seed 0 \
...
def main():
    ...
    # Create data directory where we'll store the layer-wise output tensors
    if rank == 0:
    ...
    for split, dataloader in {'train': train_dataloader, 'validation': eval_dataloader}.items():
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(pbar):
                ...
                for idx, hidden_state in enumerate(outputs[:-1]):  # indexed by layer, last layer is hidden layer before lm_head
                    ...
                    if idx <= len(attn_inputs_by_layer):
                        attn_inputs_by_layer.append([hidden_state])
                    else:
        ...
        for layer_idx, attn_inputs in enumerate(attn_inputs_by_layer):
            ...
            torch.save(attn_inputs, join(data_dir, f'attn_inputs-l={layer_idx}-s={split}.pt'))
