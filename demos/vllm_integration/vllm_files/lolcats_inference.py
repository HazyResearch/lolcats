...
class LlamaLolcatsAttentionActual(nn.Module):
    ...
    def forward(
        ...
        if seqlen == 1:
            ...
            return self.superlinear_attention(
                query, key, f_q, f_k, 
                value,
    ...
    def _prepare_lolcats_cache(self):
        if self.tp_rank == 0 and self.layer_idx == 0: print("heyo 5 -- hello prepare kv cache")
    ...
    def _init_kv_cache(self, keys, values, f_k):
        ...
        if self.tp_rank == 0 and self.layer_idx == 0: print(f"{kv_cache.shape=}")
        self.lolcats_cache.kv_cache = kv_cache
        if self.tp_rank == 0 and self.layer_idx == 0: print("heyo 4 -- bye init kv cache")
...
class LlamaLolcatsAttention(LlamaAttention):
    def __init__(self, layer_idx, *args, **kwargs):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(len(self.model.layers)):
            if i in softmax_attentions: 
                ...
                self.model.layers[i].self_attn = LlamaSdpaAttention(
                    config=self.config,
